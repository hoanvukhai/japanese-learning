...
export default function KanjiQuiz() {
  ...
  const pool = useMemo(() => {
    const baseKanji = selectedLesson === 'all'
      ? kanjiN3
      ...
      baseKanji.forEach(k => {
        k.words.forEach((w, idx) => {
          ...
          if (direction === 'forward') {
            list.push({
              id: `${k.id}_w_${idx}`,
              questionText: 'Cách đọc Hiragana của từ này là gì?',
              target: w.word,
              correctAnswer: w.hiragana,
              hint: `Nghĩa: ${meaningStr} (${hanVietWordStr})`,
            });
          } else {
            list.push({
              id: `${k.id}_w_${idx}`,
              questionText: 'Từ ghép tương ứng với cách đọc Hiragana này là gì?',
              target: w.hiragana,
        ...
        k.words.forEach((w, idx) => {
          ...
          if (direction === 'forward') {
            list.push({
              ...
              hint: `Cách đọc: ${w.hiragana} (${hanVietWordStr})`,
            });
          } else {
            list.push({
              id: `${k.id}_w_${idx}`,
  ...
  const options = useMemo(() => {
    ...
    const allAnswers = pool.map(item => item.correctAnswer);
    const otherAnswers = allAnswers.filter(ans => ans !== current.correctAnswer);
    const shuffledOthers = shuffle(Array.from(new Set(otherAnswers))).slice(0, 3);
    ...
    while (allOptions.length < 4) {
      allOptions.push(`sai_${Math.random()}`);
    }

    return shuffle(Array.from(new Set(allOptions))).slice(0, 4);
  }, [current, pool]);
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 space-y-6">
            <div>
              ...
              <select
                ...
                className="w-full py-3 px-4 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white outline-none focus:border-emerald-500 transition-colors cursor-pointer"
              >
                <option value="all">Tất cả ({pool.length} câu)</option>
                {lessons.map(l => {
                  const count = mode === 'kanji-hanviet'
                    ? kanjiN3.filter(k => k.lesson === l).length
                    : kanjiN3.filter(k => k.lesson === l).reduce((acc, k) => acc + k.words.length, 0);
                  return <option key={l} value={l}>{l} ({count} câu)</option>;
                })}
              </select>
            </div>
              ...
              {options.map((opt, i) => {
                ...
                return (
                  <button
                    ...
                    className={`p-4 rounded-2xl border-2 transition-all min-h-[4rem] flex items-center justify-center ${btnClass} ${getOptionSize(opt)}`}
                  >
                    {opt}
                  </button>
                );
