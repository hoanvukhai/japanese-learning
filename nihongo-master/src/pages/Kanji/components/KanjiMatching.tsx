...
export default function KanjiMatching({ q, showKana, level, onStateChange, onDone }: KanjiMatchingProps) {
    ...
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {tiles.map((tile, idx) => {
          ...
          else if (isSel) stateClass = 'bg-amber-50 border-amber-500 text-amber-700 dark:bg-amber-900/30 shadow-md ring-2 ring-amber-300';
          else stateClass += ' hover:border-amber-400';

          if (isFocused && !isMatched) {
            stateClass += ' ring-2 ring-amber-400 dark:ring-amber-500 border-amber-400 scale-105 z-10';
          } else if (isFocused && isMatched) {
            stateClass += ' ring-2 ring-emerald-500 border-emerald-500 scale-105 z-10 opacity-100';
          }
