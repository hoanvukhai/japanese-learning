...
export default function VocabMatching({ q, showKana, level, onStateChange, onDone }: VocabMatchingProps) {
  ...
  return (
    <div className="space-y-3">
      <div className="text-center text-sm font-semibold text-slate-500 dark:text-slate-400 mb-1">
        🔗 Nối cặp từ — <span className="text-indigo-500">{matched.size}</span>/{total} <span className="mx-1.5 text-slate-300">|</span> Sai: <span className="text-red-500">{wrongCount}</span>
