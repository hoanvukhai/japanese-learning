  ...
  return (
    <div className="space-y-3">
      <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 text-center shadow-sm border border-slate-100 dark:border-slate-700">
        <div className="text-3xl font-black text-slate-800 dark:text-white">{q.word}</div>
        {showKana && <div className="text-slate-400">{q.hiragana}</div>}
      ...
      <div className="text-center text-sm text-slate-500">
        ...
        <span className="text-xs text-slate-400">
          <kbd className="px-1 bg-slate-100 dark:bg-slate-700 rounded font-mono">←</kbd> Sai ·{' '}
          <kbd className="px-1 bg-slate-100 dark:bg-slate-700 rounded font-mono">→</kbd> Đúng
        </span>
      </div>
      {errorSelected === null && (
        ...
        <div className="space-y-2">
          <motion.div
            ...
            className={`p-3 rounded-xl text-center font-bold text-sm ${
              errorCorrect ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20' : 'bg-red-50 text-red-700 dark:bg-red-900/20'
            }`}
          >
